...
import { CanceledAndDeliveredStatus, shippingApi } from 'src/apis/shipping.api'
...
export default function Shipping() {
  ...
  const status = Number(queryParams.status) || purchaseStatus.waitForGetting
  ...
  const pickingPurchaseMutation = useMutation({
    mutationFn: shippingApi.pickingPurchase
  })

  const canceledAndDeliveredMutation = useMutation({
    mutationFn: shippingApi.CanceledAndDelivered
  })
  ...
  const handlePickingPurchase = (shipping_id: number) => {
    pickingPurchaseMutation.mutate(
      { shipping_id },
      {
        onSuccess: (result) => {
          refetch()
  ...
  const handleCanceledAndDelivered = (shipping_id: number, status: CanceledAndDeliveredStatus) => {
    canceledAndDeliveredMutation.mutate(
      { shipping_id, status },
      {
        onSuccess: (result) => {
          refetch()
  ...
  return (
    <div>
      <Helmet>
        <title>Trang shipper | Shopee Clone</title>
                ...
                <div className='flex'>
                  ...
                  <div className='ml-3 flex-grow overflow-hidden'>
                    ...
                    {purchase.status === purchaseStatus.delivered && (
                      <div className='mt-3'>
                        {t('Delivered date')}:{' '}
                        <span>{`${new Date(purchase.updated_at).getDate().toString().padStart(2, '0')}-${(new Date(purchase.updated_at).getMonth() + 1).toString().padStart(2, '0')}-${new Date(purchase.updated_at).getFullYear()}`}</span>
                      </div>
                    )}
                  ...
                  <div className='ml-1 flex-shrink-0'>
                    ...
                    {purchase.status === purchaseStatus.waitForGetting && (
                      <div className='flex justify-end'>
                        <button
                          onClick={() => handlePickingPurchase(purchase.shippings[0].shipping_id)}
                    ...
                    {purchase.status === purchaseStatus.inProgress && (
                      <div className='flex justify-center gap-3'>
                        <button
                          onClick={() =>
                            handleCanceledAndDelivered(purchase.shippings[0].shipping_id, purchaseStatus.delivered)
                          }
                          className='px-4 py-2 bg-green-600 text-white mt-5 hover:bg-opacity-80 cursor-pointer'
                        >
                          {t('Delivered')}
                        </button>
                        <button
                          onClick={() =>
                            handleCanceledAndDelivered(purchase.shippings[0].shipping_id, purchaseStatus.cancelled)
                          }
                          className='px-4 py-2 bg-red-600 text-white mt-5 hover:bg-opacity-80 cursor-pointer'
                        >
                          {t('Cancel Order')}
                        </button>
                      </div>
